...
from sqlalchemy import (
    ...
    Text, JSON, ForeignKey, Enum, create_engine, Index
)
...
class User(Base):
    ...
    admin_role = Column(String(20), nullable=True, comment="管理员角色: super/admin/viewer")
    ...
    profile_cache = relationship("UserProfileCache", back_populates="user", uselist=False)
...
class WeightRecord(Base):
    ...
    __table_args__ = (
        Index('idx_weight_record_user_time', 'user_id', 'record_time'),
    )
...
class ExerciseRecord(Base):
    ...
    is_checkin = Column(Boolean, default=False, comment="是否为打卡记录（区别于详细运动记录）")
...
class UserProfile(Base):
    ...
    motivation_type = Column(Enum(MotivationType), nullable=True, comment="动力类型（数据驱动/情感支持/目标导向）")
...
class AgentConfig(Base):
    ...
    personality_type = Column(Enum(PersonalityType), comment="性格类型（专业/温暖/活力）")
...
class UserProfileCache(Base):
    ...
    data_version = Column(DateTime, nullable=False, comment="数据版本时间戳（依赖表的最大更新时间）")
...
class SystemPrompt(Base):
    ...
    category = Column(Enum(PromptCategory), default=PromptCategory.SYSTEM, comment="分类")
...
class PromptVersion(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    prompt_id = Column(Integer, ForeignKey("system_prompts.id"), nullable=False, comment="提示词ID")
...
class SystemBackup(Base):
    ...
    status = Column(String(50), default="completed", comment="状态: pending, completed, failed")
