...
from fastapi import APIRouter, Depends, HTTPException, status, BackgroundTasks, Body, UploadFile, File, Body
...
from models.database import (
    get_db, User, ChatHistory, ConversationSummary, 
    UserProfile, AgentConfig, MessageRole, MessageType
)
...
async def build_system_prompt(user: User, db: AsyncSession) -> str:
    ...
    # 获取用户画像回答
    from models.database import ProfilingAnswer
    result = await db.execute(
        select(ProfilingAnswer).where(
            ProfilingAnswer.user_id == user.id
    ...
    # 基础信息
    prompt_parts = [
        f"你是{agent_config.agent_name if agent_config else '小助'}，用户的专属体重管理伙伴。",
        "",
        "【用户基础信息】",
    ]
    ...
    if profile:
        prompt_parts.extend([
            f"- 年龄: {profile.age or '未知'}岁",
            f"- 性别: {profile.gender or '未知'}",
            f"- 身高: {profile.height or '未知'}cm",
            f"- 基础代谢率(BMR): {profile.bmr or '未知'}",
        ])
    
    if latest_weight:
        prompt_parts.append(f"- 当前体重: {latest_weight.weight}kg")
...
async def get_recent_context(user_id: int, limit: int = 10, db: AsyncSession = None) -> List[Dict]:
    """获取最近的对话上下文"""
    result = await db.execute(
        select(ChatHistory)
...
async def send_message(
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Calling chat_with_agent for user {user_id}")
            # 调用 Agent（集成了用户画像缓存和记忆功能）
            result = await chat_with_agent(
                user_id=user_id,
                db=db,
                message=full_content
            )
            logger.info(f"chat_with_agent completed for user {user_id}")
            
            assistant_reply = result.get("response", "抱歉，我现在有点忙。")
            intermediate_steps = result.get("intermediate_steps", [])
            
            # 记录日志
            logger.info(f"LangChain Agent - User: {user_id}, Steps: {len(intermediate_steps)}")
            
        except Exception as agent_error:
            # Fallback 到旧系统（兼容性保障）
            logger.warning(f"LangChain failed, falling back to legacy AI: {agent_error}")
        ...
        # 4. 返回响应（保持与旧格式兼容，不包含 intermediate_steps）
        return {
            "success": True,
            "data": {
                "content": assistant_reply,
                "role": "assistant",
                "timestamp": datetime.utcnow().isoformat(),
                "model": "langchain-react-agent"  # 统一模型标识
            }
        }
...
async def get_daily_suggestion(
    ...
    from models.database import WeightRecord, MealRecord, ExerciseRecord, WaterRecord, MealType
    ...
    try:
        ...
        time_of_day = "早上" if 5 <= current_hour < 11 else "中午" if 11 <= current_hour < 14 else "下午" if 14 <= current_hour < 18 else "晚上"
...
直接输出建议内容，不需要解释。"""
        ...
        ai_response = await ai_service.chat([
            {"role": "system", "content": "你是专业的体重管理顾问，善于给出简洁实用的建议。"},
        ...
        elif total_water_ml < 1000 and ("水" in suggestion_content or "饮水" in suggestion_content):
            suggestion_type = "water"
            action_text = "记录饮水"
            action_target = "water.html"
        elif total_exercise_minutes < 30 and ("运动" in suggestion_content or "活动" in suggestion_content):
        ...
        suggestion = {
            ...
            "priority": "high" if not today_weight or meal_summary['total_calories'] < 500 else "medium",
            ...
            "data_summary": {
                ...
                "meals_recorded": sum([meal_summary['breakfast'], meal_summary['lunch'], meal_summary['dinner']]),
...
async def send_message_langchain(
    ...
    try:
        ...
        try:
            ...
            # 记录日志
            logger.info(f"LangChain Agent - User: {current_user.id}, Steps: {len(intermediate_steps)}")
        ...
        def format_step(step):
            ...
            if hasattr(step, 'tool') and hasattr(step, 'tool_input'):
                return {
                    ...
                    "output": getattr(step, 'tool_output', '') or getattr(step, 'log', '')
                }
        ...
        return {
            ...
            "data": {
                ...
                "intermediate_steps": [format_step(step) for step in intermediate_steps] if intermediate_steps else []
            }
        }
