...
async def get_weight_trend(
    ...
    return {
        ...
        "data": {
            ...
            "change_percent": round(change / first_weight * 100, 2) if first_weight > 0 else 0,
            "trend": trend,
...
async def get_weight_stats(
    ...
    result = await db.execute(
        select(func.avg(WeightRecord.weight), func.count(WeightRecord.id))
        .where(
            and_(
                WeightRecord.user_id == current_user.id,
    ...
    # 历史最高/最低
    result = await db.execute(
        select(func.min(WeightRecord.weight), func.max(WeightRecord.weight))
        .where(WeightRecord.user_id == current_user.id)
    )
    ...
    return {
        ...
        "data": {
            ...
            "all_time": {
                "min": min_weight,
                "max": max_weight,
                "range": round(max_weight - min_weight, 2) if min_weight and max_weight else 0
            }
        }
    }
