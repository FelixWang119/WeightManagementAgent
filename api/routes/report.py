...
from models.database import (
    get_db, User, WeeklyReport, WeightRecord, MealRecord,
    ExerciseRecord, WaterRecord, SleepRecord
)
...
async def get_ai_insights(
    ...
    # 运动习惯
    result = await db.execute(
        select(func.count(ExerciseRecord.id))
        .where(
            and_(
    ...
    # 睡眠分析
    result = await db.execute(
        select(func.avg(SleepRecord.total_minutes))
        .where(
            and_(
    ...
    insights = {
        ...
        "exercise_habit": {
            "days": exercise_count,
            "frequency": "良好" if exercise_count >= 10 else "需加强" if exercise_count >= 5 else "需改善"
        },
        "sleep_quality": {
...
async def collect_week_data(
    ...
    if weight_records:
        ...
        data["avg_weight"] = round(sum(r.weight for r in weight_records) / len(weight_records), 2)
    ...
    result = await db.execute(
        ...
        .where(
            and_(
                ...
                MealRecord.record_time >= datetime.combine(week_start, datetime.min.time()),
                MealRecord.record_time <= datetime.combine(week_end, datetime.max.time())
            )
        )
    )
    ...
    if daily_calories:
        data["avg_calories_in"] = int(sum(daily_calories.values()) / len(daily_calories))
    else:
    ...
    result = await db.execute(
        ...
        .where(
            and_(
                ...
                ExerciseRecord.record_time <= datetime.combine(week_end, datetime.max.time())
            )
        )
    )
    ...
    result = await db.execute(
        ...
        .where(
            and_(
                ...
                WaterRecord.record_time <= datetime.combine(week_end, datetime.max.time())
            )
        )
    )
    ...
    if sleep_records:
        data["sleep_avg_hours"] = round(sum(r.total_minutes for r in sleep_records) / len(sleep_records) / 60, 1)
        data["sleep_quality_avg"] = round(sum(r.quality for r in sleep_records if r.quality) / len([r for r in sleep_records if r.quality]), 1) if any(r.quality for r in sleep_records) else 0
    else:
    ...
    if data["weight_change"] < -0.3:
        data["highlights"].append(f"本周减重 {abs(data['weight_change'])}kg，保持势头！")
    
    if data["exercise_days"] >= 4:
        data["highlights"].append(f"本周运动 {data['exercise_days']} 天，非常自律！")
    
    if data["water_goal_days"] >= 5:
        data["highlights"].append(f"本周 {data['water_goal_days']} 天饮水达标， hydration 很棒！")
    
    if data["sleep_avg_hours"] >= 7:
        data["highlights"].append(f"平均睡眠 {data['sleep_avg_hours']} 小时，休息充足！")
    
    if data["weight_change"] > 0.3:
        data["improvements"].append("体重有所上升，注意控制饮食")
...
直接输出正文内容，不需要标题。"""
    ...
    try:
        messages = [
            {"role": "system", "content": "你是一位专业的体重管理教练，擅长用温暖的语言鼓励用户。"},
