...
async def get_user_profile(
    ...
    return {
        ...
        "user": {
            ...
            "vip_expire": current_user.vip_expire.isoformat()
            if current_user.vip_expire
            else None,
        },
        "profile": {
        ...
        "agent_config": {
            ...
            "personality": agent_config.personality_type.value
            if agent_config
            else "warm",
        }
        if agent_config
...
async def update_user_profile(
    ...
    bmr: Optional[int] = Body(None),
    diet_preferences: Optional[dict] = Body(None),
    exercise_habits: Optional[dict] = Body(None),
...
async def update_user_bmr(
    ...
    if bmr < 500 or bmr > 5000:
        raise HTTPException(status_code=400, detail="BMR值应在500-5000千卡/天范围内")
    ...
    return {"success": True, "message": "BMR更新成功", "bmr": bmr}
...
async def get_agent_styles(db: AsyncSession = Depends(get_db)):
    ...
    return {"success": True, "styles": await get_all_styles(db)}
