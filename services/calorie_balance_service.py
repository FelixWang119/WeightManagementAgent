...
from models.database import MealRecord, ExerciseRecord, UserProfile
    ...
    async def get_calorie_balance_chart_data(
        ...
        if chart_type == "daily":
            ...
            for month in range(3):
                ...
                for day_data in daily_data["daily_data"]:
                    day_date = datetime.strptime(day_data["date"], "%Y-%m-%d").date()
                    if day_date.year == month_date.year and day_date.month == month_date.month:
                ...
                if days_in_month > 0:
                    ...
                    monthly_data.append({
                        ...
                        "balance": int(avg_daily_burned * 30 - avg_daily_intake * 30)
                    })
