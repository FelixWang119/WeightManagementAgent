...
def record_meal_tool(
    meal_type: str, food_description: str, estimated_calories: Optional[int] = None
) -> str:
    """
    记录用户餐食。
...
def record_exercise_tool(
    exercise_type: str, duration_minutes: int, calories_burned: Optional[int] = None
) -> str:
    """
    记录用户运动。
...
class AsyncToolExecutor:
    ...
    async def _record_exercise(
        ...
        if not calories_burned:
            calories_burned = self._estimate_exercise_calories(
                exercise_type, duration_minutes
            )
    ...
    def _estimate_exercise_calories(
        self, exercise_type: str, duration_minutes: int
    ) -> int:
        """估算运动热量"""
        calories_per_hour = {
            "跑步": 600,
...
def create_tools_for_user(db, user_id):
    """
    兼容性函数：为 legacy agent 提供兼容的 create_tools_for_user 函数

    Args:
        db: 数据库会话（兼容参数，实际未使用）
        user_id: 用户ID（兼容参数，实际未使用）
    ...
    return [
        {
            "name": "record_weight",
            "description": "记录用户体重数据。当用户提到体重数值时调用。",
            "parameters": {
                "type": "object",
                "properties": {
                    "weight": {"type": "number", "description": "体重数值，单位kg"},
        ...
        {
            "name": "record_meal",
            "description": "记录用户餐食。当用户提到吃了什么食物时调用。",
            "parameters": {
                "type": "object",
                "properties": {
                    "meal_type": {
                        "type": "string",
                        "description": "餐食类型：breakfast, lunch, dinner, snack",
                    },
                    "food_description": {"type": "string", "description": "食物描述"},
                    "estimated_calories": {
                        "type": "number",
                        "description": "估算热量（千卡）",
                    },
        ...
        },
        {
            ...
            "description": "记录用户运动。当用户提到运动时调用。",
            "parameters": {
                "type": "object",
                "properties": {
                    "exercise_type": {"type": "string", "description": "运动类型"},
                    "duration_minutes": {
                        "type": "number",
                        "description": "持续时间（分钟）",
                    },
        ...
        {
            "name": "record_water",
            "description": "记录用户饮水。当用户提到喝水时调用。",
            "parameters": {
                "type": "object",
                "properties": {
                    "amount_ml": {"type": "number", "description": "饮水量（毫升）"}
                },
                "required": ["amount_ml"],
            },
        },
...
async def execute_tool(tool_name: str, arguments: dict, user_id: int, db) -> dict:
    """
    兼容性函数：为 legacy agent 提供兼容的 execute_tool 函数
    ...
    # 创建 ToolExecutor 实例并执行工具
    executor = ToolExecutor(user_id, db)
