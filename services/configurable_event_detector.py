...
class RuleEngine:
    ...
    def detect_events(self, conversation_text: str, timestamp: datetime = None) -> List[DetectedEvent]:
        """检测事件"""
        ...
        for event_type, event_config in event_types.items():
            if event_type in recognition_rules:
                events = self._detect_specific_event(
                    processed_text, event_type, event_config, 
                    recognition_rules[event_type], timestamp
                )
    ...
    def _detect_specific_event(self, text: str, event_type: str, 
                              event_config: Dict[str, Any], 
                              rules: Dict[str, Any], timestamp: datetime) -> List[DetectedEvent]:
        events = []
        
        exact_matches = self._match_exact_keywords(text, rules.get('exact_keywords', []))
        variant_matches = self._match_variant_keywords(text, rules.get('variant_keywords', []))
        pattern_matches = self._match_patterns(text, rules.get('patterns', []))
        semantic_matches = self._match_semantic_keywords(text, rules.get('semantic_keywords', []))
        ...
        if confidence >= threshold:
            ...
            event = DetectedEvent(
                ...
                keywords=rules.get('exact_keywords', []) + rules.get('variant_keywords', []),
    ...
    def _calculate_confidence(self, exact_matches: int, variant_matches: int, 
                             pattern_matches: int, semantic_matches: int, 
    ...
    def _extract_time_info(self, text: str, base_time: datetime) -> Dict[str, datetime]:
        ...
        specific_time_match = re.search(time_patterns.get('specific_time', r'(\\d{1,2}[:：]\\d{1,2})'), text)
...
class LLMFallbackDetector:
    ...
    async def detect_with_llm(self, conversation_text: str, 
    ...
    async def _simulate_llm_detection(self, text: str, timestamp: datetime) -> List[DetectedEvent]:
        return []
...
class ConfigurableEventDetector:
    ...
    async def detect_events(self, conversation_text: str, 
        ...
        needs_llm = (
            ...
            self.rule_loader.get_global_config('llm_fallback.enabled', True) and
            (not rule_based_events or max(e.confidence for e in rule_based_events) < threshold)
        )
        
        llm_events = []
        if needs_llm:
            self.logger.info("规则引擎置信度不足，触发LLM兜底检测")
            llm_events = await self.llm_detector.detect_with_llm(conversation_text, timestamp)
    ...
    def _fuse_results(self, rule_events: List[DetectedEvent], 
                     llm_events: List[DetectedEvent]) -> List[DetectedEvent]:
    ...
    async def get_conflicting_events(self, user_id: int, plan_type: str, 
                                   planned_time: datetime = None) -> List[DetectedEvent]:
        conflict_rules = self.rule_loader.get_conflict_rules()
        conflicting_types = conflict_rules.get(plan_type, {}).get('conflicting_events', [])
...
async def demo():
    ...
    for text in test_texts:
        ...
        for event in events:
            print(f"  检测到事件: {event.name} (类型: {event.type}, 置信度: {event.confidence:.2f})")
